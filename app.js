/* Aspect Ratio Cropper PWA (v3)
   - Crop-rutan är maximerad direkt vid load och när du byter ratio/orientering.
   - Porträtt är default (P). Du kan växla med knappen ↻ (P/L).
*/
...
// Base ratios (landskap som "Bredd : Höjd")
const RATIOS = [
  { label: "1:1", base: 1 / 1 },
  { label: "1.33:1", base: 1.33 / 1 },
  { label: "1.48:1", base: 1.48 / 1 },
  { label: "1.81:1", base: 1.81 / 1 },
];
...
// valt ratio (base)
let baseRatio = RATIOS[0].base;

function ratioUsed() {
  return isPortrait ? (1 / baseRatio) : baseRatio; // width / height
}

function orientationLabel() {
  return isPortrait ? "P" : "L";
}

function selectedRatioLabel() {
  const found = RATIOS.find(r => Math.abs(r.base - baseRatio) < 0.0001);
  return found ? found.label : `${baseRatio.toFixed(2)}:1`;
}

function updateRatioText() {
  ratioValue.textContent = `${selectedRatioLabel()} (${orientationLabel()})`;
}

// Crop-rect i bildens pixelkoordinater
...
// Maximerar rutan runt ett center (cx,cy) och håller den inom bilden
function maximizeRectAtCenter(cx, cy) {
  const r = ratioUsed(); // width/height

  // Hur mycket "plats" finns runt centrum innan man träffar kanterna
  const maxWCenter = 2 * Math.min(cx, imgW - cx);
  const maxHCenter = 2 * Math.min(cy, imgH - cy);

  // För ett givet ratio r gäller w <= r * h => w max blir min(maxWCenter, r * maxHCenter)
  const allowedW = Math.min(maxWCenter, r * maxHCenter);

  rect.w = Math.floor(clamp(allowedW, 50, imgW));
  rect.h = Math.floor(rect.w / r);
...
// Default/max vid load: mitt i bilden
function setInitialRectMax() {
  const cx = imgW / 2;
  const cy = imgH / 2;
  maximizeRectAtCenter(cx, cy);
}

// Vid byte av ratio/orientering: maxa runt nuvarande center
function maximizeCurrentRect() {
...
function onPointerDown(e) {
  ...
  if (pointers.size === 1) {
    if (rectContainsScreenPoint(p.x, p.y)) startDrag(e.pointerId, p);
  }
}
...
function buildRatioSheet() {
  ...
  for (const r of RATIOS) {
    ...
    right.textContent = (Math.abs(r.base - baseRatio) < 0.0001) ? "Vald" : "";
    ...
    btn.addEventListener("click", () => {
      ...
      updateRatioText();
      if (img) {
        maximizeCurrentRect(); // max direkt vid byte
        scheduleRender();
      }
      closeSheet();
      buildRatioSheet();
...
function getOutputFileName() {
  ...
  const ratioText = selectedRatioLabel().replace(":", "x");
  return `crop_${ratioText}_${orientationLabel()}_${stamp}.jpg`;
}
