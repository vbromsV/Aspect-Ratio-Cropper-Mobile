/* Aspect Ratio Cropper PWA
   - Drag: flytta den blå rutan
   - Pinch: ändra storlek (låst ratio)
   - Quick save: vanlig download
   - Export: försöker välja plats (File System Access), annars Share, annars download
*/
...
let rect = { x: 0, y: 0, w: 0, h: 0 }; // i bildens pixelkoordinater
...
function setRatio(newRatio) {
  ...
  const maxWFromH = ratio * maxHCenter;
  const allowedW = Math.min(maxWCenter, maxWFromH);

  newW = clamp(newW, 50, allowedW);
  newH = newW / ratio;

  rect.w = Math.floor(newW);
  rect.h = Math.floor(newH);
...
function onPointerUp(e) {
  ...
  if (pointers.size === 1 && img) {
    ...
    if (p && rectContainsScreenPoint(p.x, p.y)) {
      startDrag(onlyId, p);
    }
  }
}
